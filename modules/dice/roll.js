...
export class RollFFG extends Roll {
  ...
  async render(chatOptions = {}) {
    ...
    if (this?.data) {
      if (this.data.flags?.starwarsffg?.uuid) {
        ...
        if (item) {
          // See note above: never clobber the live document's system with the duplicated snapshot.
          this.data = item.toObject();
          this.data.system = await item.getItemDetails();
        }
      }
  ...
  async toMessage(messageData = {}, { messageMode = null, rollMode = null, create = true } = {}) {
    ...
    // than reimplementing visibility here (a leaked private/blind roll would be a bug).
