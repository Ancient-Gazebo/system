...
export default class EmbeddedItemHelpers {
  ...
  static async updateRealObject(temporaryItem, data) {
    ...
    // `itemmodifier`/`itemattachment`/`adjusteditemmodifer` live inside permissive AnyFields that do
    // no cleaning. Round-tripping the parent through update() can leave a nested collection (most
    // easily an empty one) stored as an index-keyed object ({0:..}) or {} instead of an array, which
    // later makes array iteration throw "is not iterable". Coerce the whole tree back to real arrays
    // before we persist so we never (re-)write the corrupted shape.
    EmbeddedItemHelpers.normalizeEmbeddedArrays(formData?.system);
  ...
  /**
   * Recursively coerce the embedded-item collections on a system-like object back into real arrays.
   * Handles object-shaped ({0:..,1:..}) and empty ({}) corruption, recursing through nested
   * attachments/modifiers. Idempotent and safe on already-correct data.
   * @param {object} system - a `system`-like object that may directly contain itemmodifier/itemattachment keys
   * @returns {object} the same object, with collections normalized in place
   */
  static normalizeEmbeddedArrays(system) {
    if (!system || typeof system !== "object") return system;
    // An embedded entry's document type mirrors the collection key it lives under (adjusteditemmodifer
    // is a parallel list of itemmodifier-shaped entries). A write round-trip can drop that top-level
    // `type`, which later leaves the editor resolving `ffg-embedded-undefined.html`; restore it here.
    const TYPE_BY_KEY = {
      itemmodifier: "itemmodifier",
      adjusteditemmodifer: "itemmodifier",
      itemattachment: "itemattachment",
    };
    for (const key of Object.keys(TYPE_BY_KEY)) {
      if (system[key] != null && !Array.isArray(system[key])) {
        system[key] = Object.values(system[key]);
      }
      if (Array.isArray(system[key])) {
        for (const entry of system[key]) {
          if (entry && typeof entry === "object" && !entry.type) {
            entry.type = TYPE_BY_KEY[key];
          }
          EmbeddedItemHelpers.normalizeEmbeddedArrays(entry?.system);
        }
      }
    }
    return system;
