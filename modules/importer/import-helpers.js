...
export default class ImportHelpers {
  ...
  static async getCompendiumPack(type, name) {
    ...
    if (!pack) {
      ...
      try {
        ...
        // A concurrent import phase may have created this pack between our check and now.
        // Reuse the existing pack instead of aborting the import.
  ...
  static async processDieMod(mod) {
    ...
    await ImportHelpers.asyncForEach(mod.DieModifier, async (dieMod) => {
      ...
      if (!dieMod) {
        ...
        // processSkillMod returns undefined for mods that don't map to a known skill; skip those
        if (skillModifier) output.attributes[key] = skillModifier.value;
  ...
  static async getTemplate(type) {
    // The registered DataModel is now the single source of truth for a type's data
    // shape (template.json no longer carries field definitions). Build a blank,
    // default-populated system object from the model. This produces the same shape
    // the resolved template.json used to return. Falls back to {} for unknown types.
    const cls = CONFIG.Item?.dataModels?.[type] || CONFIG.Actor?.dataModels?.[type];
    if (cls) {
      try {
        return new cls({}, { strict: false }).toObject();
      } catch (e) {
        CONFIG.logger?.warn?.(`getTemplate: could not build '${type}' from its DataModel`, e);
      }
    }
    return {};
  }
  ...
  static async createActiveEffects(item) {
    if (["species", "gear", "weapon", "armour", "shipattachment"].includes(item.type)) {
      ...
      if (!inherentEffect) {
        ...
        await item.createEmbeddedDocuments("ActiveEffect", [effects], { noHook: true });
      }
    }
  }
   ...
   * @type {*|{}}
  */
  // NOTE: ActiveEffect writes below pass { noHook: true }. During import these effects
  // live on actor-less compendium items, so third-party AE-animation modules (e.g.
  // Automated Animations) crash dereferencing a null actor in their create/update/delete
  // hooks. Suppressing the document hooks for import-only writes avoids that; the
  // system's own updateActiveEffect hook is a no-op for these changes-only updates.
  static async applyActiveEffectOnUpdate(item, formData) {
    ...
    if (item.type === "career" && inherentEffect) {
      ...
      await inherentEffect.update({changes: changes}, { noHook: true });
    ...
    if (toCreate.length) {
      await item.createEmbeddedDocuments("ActiveEffect", toCreate, { noHook: true });
    }

    if (toDelete.length) {
      await item.deleteEmbeddedDocuments("ActiveEffect", toDelete, { noHook: true });
    }
  ...
  static async applyTalentActiveEffects(specialization) {
    ...
    if (toCreate.length) {
      await specialization.createEmbeddedDocuments("ActiveEffect", toCreate, { noHook: true });
    }
  }
}
