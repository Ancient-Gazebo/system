...
export class ItemFFG extends ItemBaseFFG {
  ...
  async _onCreate(data, options, user) {
    ...
    if (this.type === "species" && this.isEmbedded && this.actor) {
      ...
      if (inherent) {
        ...
        const rawStrain = parseInt(attrs.Strain?.value, 10);
        const newChanges = foundry.utils.deepClone(inherent.changes);
        let dirty = false;
        for (const change of newChanges) {
          let want;
          if (change.key === "system.stats.wounds.max" && Number.isFinite(rawWounds)) {
            want = rawWounds + speciesBrawn;
          ...
          if ((parseInt(change.value, 10) || 0) !== want) {
            change.value = want;
            dirty = true;
          }
        }
        if (dirty) await inherent.update({ changes: newChanges });
      }
    }
  }
  ...
  async getItemDetails() {
    ...
    // Replace the {ranks} placeholder in talent descriptions with the talent's rank count, so it
    // renders as a number (e.g. "suffer up to 2 Strain") instead of the literal "{ranks}". This
    // primarily covers standalone/owned talents (those not granted through a specialization tree).
    // Prefer the actor's aggregated rank for this talent - matching the number shown in the talent
    // list and covering a talent learned from more than one source - and fall back to the item's
    // own current rank. `prettyDesc` is already-rendered HTML, but {ranks} is a plain literal that
    // survives dice rendering untouched, so substituting there too keeps every consumer in sync.
    //
    // Also handles the suffixed form {ranks|word} (e.g. "{ranks|times}"), which renders the rank
    // number followed by the word after the pipe -> "4 times". The suffix is whatever text follows
    // the pipe, trimmed (so {ranks|times}, {ranks | times}, etc. all work).
    if (this.type === "talent") {
      const listEntry = this.actor?.talentList?.find((t) => (this.id && t.itemId === this.id) || t.name === this.name);
      const rankCount = Number.isFinite(Number(listEntry?.rank)) ? Number(listEntry.rank) : Number(this.system?.ranks?.current);
      if (Number.isFinite(rankCount)) {
        const ranksPattern = /\{\s*ranks\s*(?:\|([^}]*))?\}/gi;
        const substitute = (text) => String(text ?? "").replace(ranksPattern, (match, suffix) => {
          const word = (suffix ?? "").trim();
          return word ? `${rankCount} ${word}` : `${rankCount}`;
        });
        data.description = substitute(data.description);
        if (typeof data.prettyDesc === "string") {
          data.prettyDesc = substitute(data.prettyDesc);
        }
      }
    }
