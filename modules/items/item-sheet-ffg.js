...
export class ItemSheetFFG extends FFGDocumentSheet {
  static DEFAULT_OPTIONS = {
    // `v2` is required, not cosmetic: the stylesheets key item-sheet layout off
    // `.starwarsffg.sheet.item.v2`. The V2-styled variant was the makeDefault
    // sheet pre-migration, so the collapsed sheet keeps the class. The initial
    // tab stays "description" (this fork opens item descriptions by default).
  ...
  /** Window title is just the item name. @override */
  get title() {
    return this.item.name;
  }

  /** Alias for the underlying document. */
  get item() {
    return this.object;
  }

  /** The owning actor, if this item is embedded. */
  get actor() {
   ...
   * A handful of types map to a shared/legacy class name rather than the
   * default `item-sheet-<type>`.
   * @override
   */
  ...
  /**
   * Type-specific interactive-resize floors; tree-grid types (specialization /
   * force power / signature ability) need a larger floor to keep the grid
  ...
  setPosition(position = {}) {
    ...
    // Remember user-driven resizes so subsequent re-renders (e.g. editing a custom modifier's
    // type/target/value, which re-runs getData) don't snap the sheet back to its hardcoded
  ...
  async getData(options) {
    ...
    // body below and the templates read `data.item` (V1 ItemSheet used to
    // provide it).
    ...
    // `toObject` only serializes schema fields, dropping derived data (renderedDesc,
    // adjusted values, ...) once a DataModel is registered. Source the mutable display
    // copy from the live prepared system instead.
    data.data = preparedSystemCopy(this.object);
    ...
    // The tabs partial branches on `contains cls "V2"` to render the icon tab
    // strip. After the V2-full collapse the real class is `ItemSheetFFG` (no
    // "V2" suffix), so carry the V2 marker explicitly - this IS the V2 sheet.
    // Without it, weapon/armour/gear tabs fall back to the retired text style.
    ...
    // The switch above sets a per-type default size. If the user has manually resized this sheet,
  ...
  _getSummarizedQualities(data) {
    ...
    if (Object.keys(data.data).includes("itemmodifier")) {
      for (const mod of data.data.itemmodifier) {
        // respect the per-modification "show in qualities" toggle (absent/true => shown, so existing
        // data is unchanged); lets non-quality modifiers like a raw Damage bonus apply without
        // cluttering the qualities list
        if (mod.system?.showInQualities === false) {
          continue;
        }
      ...
      for (const attachment of data.data.itemattachment) {
        ...
        if (Object.keys(attachment.system).includes("itemmodifier")) {
          for (const mod of attachment.system.itemmodifier) {
            ...
            // respect the per-modification "show in qualities" toggle (absent/true => shown)
            if (mod.system?.showInQualities === false) {
              continue;
            }
            const modName = mod.name;
  ...
  activateListeners(html) {
    ...
    html.find(".ffg-refund").click(async (ev) => {
      if(this.actor && !this.actor?.verifyEditModeIsNotEnabled()) return;
      await this._handleItemRefund(ev)
    });
    ...
    if (["gear", "weapon", "armour", "shipweapon"].includes(this.object.type)) {
      this.sheetoptions = new ItemOptions(this, html);
      // Price, rarity, and the cybernetic toggle only make sense for carried gear/weapons/armour,
      // not for vehicle-mounted weapons.
      if (["gear", "weapon", "armour"].includes(this.object.type)) {
        ...
        this.sheetoptions.register("countsAsCybernetic", {
          name: game.i18n.localize("SWFFG.SheetOptions2.countsAsCybernetic.Name"),
          hint: game.i18n.localize("SWFFG.SheetOptions2.countsAsCybernetic.Hint"),
          type: "Boolean",
          default: false,
        });
      }
      ...
      // Quantity tracking applies to both personal weapons (grenades, charges) and vehicle
      // weapons (missiles, torpedoes).
      if (["weapon", "shipweapon"].includes(this.object.type)) {
        this.sheetoptions.register("enableQuantity", {
          name: game.i18n.localize("SWFFG.SheetOptions2.enableQuantity.Name"),
          hint: game.i18n.localize("SWFFG.SheetOptions2.enableQuantity.Hint"),
          type: "Boolean",
          default: false,
        });
      }
    ...
    // (Legacy manual tab activation removed: FFGDocumentSheet._activateCoreListeners
    // binds Tabs from DEFAULT_OPTIONS.tabs and persists the active tab per document.)
      ...
      html.find(".expand-desc").click(async (ev) => {
        ...
        if (!$(ev.target).hasClass("fa-trash") && !$(ev.target).hasClass("fas") && !$(ev.target).hasClass("rollable")) {
          ...
          if (!$(ev.target).hasClass("fa-trash")) {
            ...
            // Embedded mods/attachments live in permissive AnyField arrays, so a prior write round-trip
            // can drop the entry's top-level `type`. Without it the editor resolves the template path to
            // `ffg-embedded-undefined.html` and fails to open. The collection key the entry was found
            // under (`clickedType`) is its authoritative embedded type, so backfill from it.
            if (clickedObject && !clickedObject.type) {
              clickedObject.type = clickedType;
            }
    ...
    html.find(".item-delete").click(async (ev) => {
      ...
      if (itemType === "ability") {
        ...
        await this._deleteAbility(item, ev);
      } else if (itemType === "talent") {
        const item = this.item.system.talents[itemId];
        await this._deleteTalent(item, ev);
      }
    });
    ...
    // Allow viewing/editing an attachment's optional modifications while it is standalone in
    // the items tab (before it is dragged onto a weapon/armour/vehicle). Reuses the embedded
    // editor. For ship attachments this also works once placed on a vehicle, since those are
    // real owned items edited through this same sheet.
    if (["itemattachment", "shipattachment"].includes(this.object.type)) {
      html.find(".edit-modifications").on("click", (ev) => {
        ev.preventDefault();
        const typeChoices = {};
        for (const key of Object.keys(CONFIG.FFG.itemmodifier_types)) {
          const entry = CONFIG.FFG.itemmodifier_types[key];
        ...
        const data = {
          sourceObject: this.object,
          clickedObject: this.object.toObject(),
          typeChoices: typeChoices,
          standalone: true,
        };
        new itemEditor(data).render(true);
    ...
    html.find(".item-delete").on("click", async (event) => {
      ...
      // also matches species talent/ability pills, which are keyed by data-item-id under
      // system.talents / system.abilities and removed by their own delete handler above. Bail out for
      // anything that isn't an indexed array entry, otherwise `system[itemType][itemIndex]` below
      // dereferences undefined and throws "Cannot read properties of undefined (reading 'undefined')".
    ...
    html.find(".item-pill .rank").on("click", async (event) => {
      event.preventDefault();
      event.stopPropagation();
      ...
      if (item) {
        ...
        // DialogV2 takes an HTML string, so pre-render the per-type edit template.
        const editContent = await foundry.applications.handlebars.renderTemplate(
          `systems/starwarsffg/templates/items/dialogs/ffg-edit-${itemType}.html`,
          { title, content: { item, type: itemType, parenttype: this.object.type } }
        );
  ...
  async _handleItemRefund(event) {
    event.preventDefault();
    event.stopPropagation();
    const action = $(event.target).data("refund-action");
    if (action === "specialization-upgrade") {
      await this._refundTreeNode(event, "specialization");
    } else if (action === "forcepower-upgrade") {
      await this._refundTreeNode(event, "forcepower");
    } else if (action === "signatureability-upgrade") {
      await this._refundTreeNode(event, "signatureability");
    }
  }

  /**
   * Resolve the owning actor for this (owned) tree item, mirroring the checks used by purchases.
   * @returns {Actor|null}
   */
  _resolveTreeOwner() {
    const ownerFlag = this.object.flags?.starwarsffg?.ffgUuid;
  ...
  /**
   * Locate the most recent un-refunded XP-log entry that paid for this node, so we can refund the
   * exact amount that was spent (handling mentor discounts, and refunding 0 for talents that were
   * granted or auto-purchased for free across trees). Returns the matched entry or null.
   */
  _findPurchaseLogEntry(owner, description) {
    const log = owner.getFlag("starwarsffg", "xpLog") || [];
    return log.find((entry) => entry.action === "purchased" && !entry.refunded && entry.description === description) || null;
  }

  /**
   * Refund a learned specialization talent, Force power upgrade, or signature ability upgrade. The
   * refund is blocked if it would disconnect any other learned node from the tree's entry (top row
   * / basic power / base ability).
   *
   * @param {Event} event
   * @param {string} type  "specialization", "forcepower", or "signatureability"
   */
  async _refundTreeNode(event, type) {
    const element = $(event.target);
    const nodeId = element.data("upgrade-id");
    const baseName = element.data("base-item-name");
    const nodeName = element.data("upgrade-name");
    const collection = type === "specialization" ? "talents" : "upgrades";
    const node = this.object.system[collection]?.[nodeId];
    if (!node || !(node.islearned === true || node.islearned === "true")) {
      return;
    }

    const owner = this._resolveTreeOwner();
    if (!owner) {
    ...
    const impact = TalentTree.refundImpact(this.object.system[collection], nodeId, type);
    if (impact.orphaned) {
      const orphanNames = impact.orphans
        .map((k) => this.object.system[collection]?.[k]?.name)
        .filter((n) => n);
    ...
    // Work out how much to refund from the XP log (0 if it was free / granted / auto-purchased).
    // The description must match exactly what the corresponding buy path logged.
    let logDescription;
    if (type === "specialization") {
      logDescription = `specialization ${baseName} upgrade ${nodeName}`;
    } else if (type === "signatureability") {
      logDescription = `signature ability ${baseName} upgrade ${nodeName}`;
    } else {
      logDescription = `force power ${baseName} upgrade ${nodeName}`;
    }
    const logEntry = this._findPurchaseLogEntry(owner, logDescription);
    const refundAmount = logEntry ? parseInt(logEntry.xp.cost, 10) || 0 : 0;

    const dialog = DialogV2.wait({
        window: { title: game.i18n.localize("SWFFG.Actors.Sheets.Refund.DialogTitle") },
        classes: ["dialog", "starwarsffg"],
        content: `<p>${game.i18n.format("SWFFG.Actors.Sheets.Refund.ConfirmText", { talent: nodeName, cost: refundAmount })}</p>`,
        buttons: [
          {
            action: "done",
            ...
            callback: async (event, button, dialog) => {
              ...
              if (refundAmount > 0) {
                ...
                const log = owner.getFlag("starwarsffg", "xpLog") || [];
                if (logEntry) {
                  // Mark the original purchase as refunded so it can't be refunded twice.
              ...
              if (this.object.type === "specialization" && owner) {
                await ItemHelpers.syncActorTreeAEs(owner);
              }
            },
          },
          {
  ...
  async _buyForcePowerUpgrade(event) {
    ...
    const dialog = DialogV2.wait({
        ...
        classes: ["dialog", "starwarsffg"],
        content: `<p>${game.i18n.format("SWFFG.Actors.Sheets.Purchase.FP.ConfirmText", {cost: cost, upgrade: upgradeName})}</p>`,
        buttons: [
          {
            action: "done",
  ...
  async _buySignatureAbilityUpgrade(event) {
    ...
    const dialog = DialogV2.wait({
        ...
        classes: ["dialog", "starwarsffg"],
        content: `<p>${game.i18n.format("SWFFG.Actors.Sheets.Purchase.SA.ConfirmText", {cost: cost, upgrade: upgradeName})}</p>`,
        buttons: [
          {
            action: "done",
            icon: "fa-regular fa-circle-up",
  ...
  async _buySpecializationUpgrade(event) {
    ...
    const dialog = DialogV2.wait({
        ...
        classes: ["dialog", "starwarsffg"],
        content: `<p>${game.i18n.format("SWFFG.Actors.Sheets.Purchase.Specialization.ConfirmText", {cost: cost, upgrade: upgradeName})}</p>`,
        buttons: [
          {
            action: "done",
            icon: "fa-regular fa-circle-up",
            ...
            const that = $(dialog.element);
              ...
              input.checked = true;
              await this._onSubmit(new Event("submit", { cancelable: true }), { render: true });
              // FFG rule: reaching an unranked talent already owned in another tree auto-purchases
              // it (for free) on this tree. Buying this talent may have connected to such a talent.
              try {
                await ActorHelpers.autoPurchaseConnectedTalents(owner);
              } catch (e) {
                CONFIG.logger.warn("Failed to auto-purchase connected cross-tree talents", e);
              }
              // Re-check duplicate suppression across every tree, not just this one - an unranked
              // talent learned in multiple trees must only apply its modifiers once. Running this
              // on every purchase also self-heals characters affected by the historical
              // double-application bug.
              try {
                await ItemHelpers.syncActorTreeAEs(owner);
              } catch (e) {
                CONFIG.logger.warn("Failed to sync specialization Active Effects after purchase", e);
              }
            },
          },
          {
  ...
  async _updateObject(event, formData, { render = false } = {}) {
    ...
    // Return the promise so callers that `await sheet.submit()` actually wait for the document
    // write (and re-render) to finish. Without this, submit() resolves before the talent's
    // islearned change is persisted, which made cross-tree auto-purchase read stale data and
    // only "catch up" on the following purchase. The `render` intent comes from the base's
    // manual submit pipeline and flows through to document.update (default: no re-render).
    return itemUpdate(event, formData, { render });
  }
  ...
  async _onDropItem(event) {
    ...
    if ((itemObject.type === "itemattachment" || itemObject.type === "itemmodifier") && ((obj.type === "shipweapon" && itemObject.system.type === "weapon") || obj.type === itemObject.system.type || itemObject.system.type === "all" || obj.type === "itemattachment")) {
      // Deep-clone the existing collection before mutating it. `obj.system[type]` is backed by the
      // document source (the itemmodifier/itemattachment AnyField arrays aren't cloned on prepare),
      // so bumping an existing entry's rank in place and handing the same array back to update()
      // leaves the payload identical to _source - Foundry's diff finds no change, so the rank bump
      // doesn't re-render the sheet (it only appeared after reopening). The first drop re-renders
      // because it pushes a new element and transfers Active Effects; subsequent rank-only bumps did
      // not. Cloning makes the bump a real diff. Mirrors the modifications editor's _updateObject.
      let items = foundry.utils.deepClone(obj?.system?.[itemObject.type] ?? []);
      ...
      switch (itemObject.type) {
        ...
        case "itemattachment": {
          if (this.object.system.hardpoints.adjusted - itemObject.system.hardpoints.value >= 0) {
            ...
            // start uninstalled and can be toggled on via the attachment editor.
  ...
  async _transferActiveEffects(droppedItem) {
    ...
    // NOTE: talent -> species is intentionally NOT transferred. When a species is added to a character
    // the createItem hook grants each species talent as its own talent item, which already carries the
    // talent's modifier Active Effect. Copying that AE onto the species as well made the modifier apply
    // twice (e.g. Gearhead). Abilities stay - their granted items are built without AEs, so the copy on
    // the species is the only application. Specialization trees manage their talent AEs via syncAEStatus,
    // so that transfer is likewise still needed.
    if (
