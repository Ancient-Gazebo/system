...
import { FFG_ACTOR_DATAMODELS, FFG_ITEM_DATAMODELS } from "./datamodels/index.js";
...
Hooks.once("init", async function () {
  ...
  // Register faithful system DataModels. These mirror the historical template.json
  // shapes field-for-field (dynamic maps -> ObjectField), so registering them is
  // non-lossy for existing data while enabling typed access and per-type schemas.
  ...
  game.settings.register("starwarsffg", "confirmItemDelete", {
    name: game.i18n.localize("SWFFG.Settings.ConfirmItemDelete.Name"),
    hint: game.i18n.localize("SWFFG.Settings.ConfirmItemDelete.Hint"),
    scope: "client",
    config: true,
    default: true,
    type: Boolean,
  ...
  // define custom status effects
    ...
    // like the other "Next Check" statuses). Pips only matter on rolls that include Force dice;
    // on a non-Force check the pip is shown but inert, and the status is still consumed.
    CONFIG.statusEffects.push({
      id: "starwarsffg-pull-force-dark",
    ...
    CONFIG.statusEffects.push({
      id: "starwarsffg-pull-force-light",
    ...
    const genericConditions = [
      { id: "bleeding",     img: "icons/svg/blood.svg",         name: "SWFFG.Status.Bleeding" },
      { id: "blinded",      img: "icons/svg/blind.svg",         name: "SWFFG.Status.Blinded" },
      { id: "burning",      img: "icons/svg/fire.svg",          name: "SWFFG.Status.Burning" },
      { id: "charmed",      img: "icons/magic/control/hypnosis-mesmerism-pendulum.webp", name: "SWFFG.Status.Charmed" },
      { id: "frightened",   img: "icons/svg/terror.svg",        name: "SWFFG.Status.Frightened" },
      { id: "diseased",     img: "icons/svg/biohazard.svg",     name: "SWFFG.Status.Diseased" },
      { id: "flying",       img: "icons/svg/wing.svg",          name: "SWFFG.Status.Flying" },
      { id: "hovering",     img: "icons/svg/up.svg",            name: "SWFFG.Status.Hovering" },
      { id: "grappled",     img: "icons/svg/trap.svg",          name: "SWFFG.Status.Grappled" },
      { id: "restrained",   img: "icons/svg/net.svg",           name: "SWFFG.Status.Restrained" },
      { id: "invisible",    img: "icons/svg/invisible.svg",     name: "SWFFG.Status.Invisible" },
      { id: "poisoned",     img: "icons/svg/poison.svg",        name: "SWFFG.Status.Poisoned" },
      { id: "sleeping",     img: "icons/svg/sleep.svg",         name: "SWFFG.Status.Sleeping" },
      { id: "unconscious",  img: "icons/svg/unconscious.svg",   name: "SWFFG.Status.Unconscious" },
      { id: "frozen",       img: "icons/svg/frozen.svg",        name: "SWFFG.Status.Frozen" },
      { id: "blessed",      img: "icons/svg/angel.svg",         name: "SWFFG.Status.Blessed" },
      { id: "stunned",      img: "icons/svg/daze.svg",          name: "SWFFG.Status.Stunned" },
      { id: "regenerating", img: "icons/svg/regen.svg",         name: "SWFFG.Status.Regenerating" },
      { id: "degenerating", img: "icons/svg/degen.svg",         name: "SWFFG.Status.Degenerating" },
      { id: "fireShield",   img: "icons/svg/fire-shield.svg",   name: "SWFFG.Status.FireShield" },
      { id: "iceShield",    img: "icons/svg/ice-shield.svg",    name: "SWFFG.Status.IceShield" },
      { id: "magicShield",  img: "icons/svg/mage-shield.svg",   name: "SWFFG.Status.MagicShield" },
      { id: "silenced",     img: "icons/svg/silenced.svg",      name: "SWFFG.Status.Silenced" },
      { id: "shocked",      img: "icons/svg/lightning.svg",     name: "SWFFG.Status.Shocked" },
      { id: "corroding",    img: "icons/svg/acid.svg",          name: "SWFFG.Status.Corroding" },
      { id: "deaf",         img: "icons/svg/deaf.svg",          name: "SWFFG.Status.Deaf" },
      { id: "paralyzed",    img: "icons/svg/paralysis.svg",     name: "SWFFG.Status.Paralyzed" },
      { id: "empowered",    img: "icons/svg/upgrade.svg",       name: "SWFFG.Status.Empowered" },
  ...
  Handlebars.registerHelper("ffgDiceSymbols", function (text) {
    return PopoutEditor.renderDiceImagesSync(text, {});
  });
...
Hooks.once("ready", async () => {
  ...
  Hooks.on("createItem", async (item, options, userId) => {
    ...
    if (item.isEmbedded && item.parent.documentName === "Actor") {
      ...
      // FFG rule: acquiring a new specialization may give free access to unranked talents already
      // owned in another tree (e.g. a shared talent sitting in the new tree's top tier). Resolve
      // those now so the character doesn't have to buy them again.
      if (item.type === "specialization" && actor.type === "character") {
        try {
          await ActorHelpers.autoPurchaseConnectedTalents(actor);
        } catch (e) {
          CONFIG.logger.warn("Failed to auto-purchase connected cross-tree talents on specialization add", e);
        }
      }
    }
  });
  ...
  const defaultDestinyMenu = [
    ...
    {
      name: game.i18n.localize("SWFFG.CriticalRoller.MenuEntry"),
  ...
  // Status Icon Counters support is handled in ActiveEffectFFG#apply, which scales additive changes
  // by the effect's stack count (flags.statuscounter.value) live during data preparation. That keeps
  // the base "+1" changes intact and re-derives the count on every prep, so no flag-watching hook is
  // needed here - core re-prepares and re-renders the actor whenever the counter flag changes.
...
async function registerCrewRoles() {
  ...
  // Master list of languages selectable on actor sheets. Edited via the LanguageSettings menu.
  const defaultArrayLanguages = [
    "Galactic Basic",
    "Binary",
    "Bocce",
    "Bothese",
  ...
  game.settings.registerMenu("starwarsffg", "arrayLanguages", {
    name: game.i18n.localize("SWFFG.Languages.Settings.Name"),
    label: game.i18n.localize("SWFFG.Languages.Settings.Label"),
    hint: game.i18n.localize("SWFFG.Languages.Settings.Hint"),
    icon: "fas fa-language",
    type: LanguageSettings,
    restricted: true,
  });
  game.settings.register("starwarsffg", "arrayLanguages", {
    module: "starwarsffg",
    name: "arrayLanguages",
    scope: "world",
    default: defaultArrayLanguages,
    config: false,
    type: Object,
  });

  // Currency denominations. Edited via the CurrencySettings menu; loaded into CONFIG.FFG below so
  // the manager, sheets, and other modules all read a single live config (mirrors CONFIG.DND5E).
  game.settings.registerMenu("starwarsffg", "currencies", {
    name: game.i18n.localize("SWFFG.Currency.Settings.Name"),
    label: game.i18n.localize("SWFFG.Currency.Settings.Label"),
    hint: game.i18n.localize("SWFFG.Currency.Settings.Hint"),
    icon: "fas fa-coins",
    type: CurrencySettings,
    restricted: true,
  });
  game.settings.register("starwarsffg", "currencies", {
    module: "starwarsffg",
    name: "currencies",
    scope: "world",
    default: {},
  ...
  game.settings.register("starwarsffg", "defaultCurrency", {
    module: "starwarsffg",
  ...
  // Overlay any GM-configured currencies onto the in-memory config. An empty setting means "use the
  // shipped default" (a single "glass" denomination defined in config/ffg-currency.js).
  try {
    const storedCurrencies = game.settings.get("starwarsffg", "currencies");
    if (storedCurrencies && !foundry.utils.isEmpty(storedCurrencies) && CONFIG.FFG) {
      CONFIG.FFG.currencies = storedCurrencies;
    }
    const storedDefault = game.settings.get("starwarsffg", "defaultCurrency");
    if (storedDefault && CONFIG.FFG?.currencies?.[storedDefault]) {
      CONFIG.FFG.defaultCurrency = storedDefault;
    }
  } catch (e) {
    CONFIG.logger?.warn?.(`Unable to load currency configuration: ${e}`);
  }
}
